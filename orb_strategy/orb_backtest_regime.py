...
def backtest_regime(days, p, orb_m, target_R, risk, max_Lev, AUM_0, commission, 
    ...
    for t in range(1, len(days)):
        ...
        # VIX <= 20: Allow ONLY Longs
        
        if current_vix > 20:
            if side == 1: # Long Signal
                side = 0 # Ignored
        else: # VIX <= 20
            if side == -1: # Short Signal
                side = 0 # Ignored
...
def run_regime_analysis():
    ...
    # Run Combined (Baseline for comparison: ADX>20 & VIX<=20)
    # But wait, the previous "Combined" filter simply REMOVED high VIX days.
    # It didn't allow Shorts in High VIX.
    # So we should compare this "Regime" strategy to the "Combined Filter" strategy.
    
    print("\n--- Running Combined Filter (Reference) ---")
    # Reference: ADX>20, VIX<=20 (Long/Short Allowed, but High VIX skipped)
    ...
    # VIX <= 20: Long Only
    # VIX > 20: Short Only
    res_regime = backtest_regime(days, df, orb_m, target_R, risk, max_Lev, AUM_0, commission, 
                                vix_map, adx_threshold=20)
    ...
    print(f"\n--- Regime Strategy (Split at VIX 20) Performance ({ticker}) ---")
    print(stats_regime.to_string(index=False))
    
    # Plot
    plt.figure(figsize=(12, 6))
    plt.plot(res_combined['Date'], res_combined['AUM'], label='Combined Filter (No High VIX)', alpha=0.7)
    plt.plot(res_regime['Date'], res_regime['AUM'], label='Regime (Long<=20, Short>20)', linewidth=2, color='purple')
    plt.yscale('log')
    plt.title(f'5-min ORB Strategy: Combined Filter vs Regime Switching\nTicker: {ticker} | ADX>20')
    plt.grid(True, which="both", ls="--")
    plt.legend()
    plt.savefig('orb_strategy/comparison_regime.png')
    print("Saved comparison chart to orb_strategy/comparison_regime.png")
