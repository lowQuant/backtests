...
def summary_statistics(dailyReturns, pnl_r=None):
    ...
    if pnl_r is not None:
        ...
        if len(trades) > 0:
            ...
            metrics.extend(["Win Rate (%)", "Avg Win (R)", "Avg Loss (R)", "Trade Count"])
            values.extend([win_rate*100, avg_win, avg_loss, len(trades)])
...
def run_analysis(ticker, filepath):
    ...
    # Plot
    plot_equity_curve(str_df, AUM_0, orb_m, target_R, ticker, 'orb_strategy')

if __name__ == "__main__":
    os.makedirs('orb_strategy', exist_ok=True)
    
    # QQQ
    run_analysis('QQQ', 'data/QQQ_20200102_20251230.parquet')
    
    # TQQQ
    # Check filename
    run_analysis('TQQQ', 'data/TQQQ_intraday_2020-2026.parquet')
