"""
Tom Basso Channel Strategy - Mean Reversion Short

Different approach for shorts: Mean reversion instead of trend following
- Entry: RSI(2) > 90 (extremely overbought)
- Exit: RSI(2) < 50 OR after 5 days max hold

This captures short-term overbought reversions rather than fighting the trend.
"""
...
# Mean reversion short params
RSI_PERIOD = 2
RSI_ENTRY = 90   # Short when RSI > 90
RSI_EXIT = 50    # Cover when RSI < 50
MAX_HOLD_DAYS = 5  # Max hold period for shorts
...
def prepare_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculate RSI(2) for short signal
    df['RSI_2'] = df.groupby('Symbol')['Close'].transform(
    ...
    # Short entry signal: RSI > 90
    df['short_signal'] = np.where(df['RSI_2'] > RSI_ENTRY, -1, 0)
...
def run_portfolio_backtest(df: pd.DataFrame, mode: str = 'long') -> dict:
    """
    Run day-by-day portfolio backtest with mean reversion short.
    """
    ...
    for date in tqdm(dates, desc=f"{mode.upper()} simulation"):
        ...
        current_rsi = day_data.set_index('Symbol')['RSI_2'].to_dict()
        ...
        for sym, pos in positions.items():
            ...
            if pos['direction'] == 1:  # Long exit
                ...
                if long_sig != 1:
                    should_exit = True
            else:  # Short exit - mean reversion logic
                rsi = current_rsi.get(sym, 50)
                days_held = (date - pos['entry_date']).days
                
                # Exit conditions for short:
                # 1. RSI dropped below 50 (mean reversion target)
                # 2. Max hold period reached
                if rsi < RSI_EXIT or days_held >= MAX_HOLD_DAYS:
                    should_exit = True
...
def main():
    print("=" * 70)
    print("TOM BASSO CHANNEL STRATEGY - MEAN REVERSION SHORT")
    print("=" * 70)
    print(f"Starting Capital: ${STARTING_CAPITAL:,}")
    print(f"Position Size: {POSITION_SIZE_PCT*100:.0f}% of equity")
    print(f"LONG: {LONG_LOOKBACK}d channels, 150d SMA filter")
    print(f"SHORT: RSI({RSI_PERIOD}) > {RSI_ENTRY} entry, RSI < {RSI_EXIT} or {MAX_HOLD_DAYS}d max hold exit")
    ...
    # Summary comparison
    print("\n" + "=" * 90)
    print("SUMMARY COMPARISON (MEAN REVERSION SHORT)")
    print("=" * 90)
    ...
    plt.title(f'Tom Basso Channel Strategy - MEAN REVERSION SHORT\n(Long: 200d channels, Short: RSI(2) > {RSI_ENTRY})', fontsize=14, fontweight='bold')
    ...
    plot_path = output_dir / "backtest_meanrev_short.png"
    ...
    results_path = output_dir / "backtest_meanrev_short_results.csv"
