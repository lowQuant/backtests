...
def prepare_data(df):
    ...
    df['ADX'] = g.apply(calc_adx_group).reset_index(level=0, drop=True)
...
def run_simulation(df, trade_pct_equity, sl_config=None):
    ...
    for date in tqdm(sim_dates, desc="Simulating Days"):
        ...
        for pos in positions:
            ...
            # If no pending exit, check stops and new signals
            if exit_price is None:
                ...
                # 2. Check for EXIT SIGNALS (to be executed NEXT OPEN)
                elif close < ema_exit:
                    pos['Pending_Exit'] = True
                    pos['Pending_Exit_Reason'] = "Cross Under EMA5 (Target Met)"
                
                # 3. Time Stop
                else:
                    pos['Days_Held'] += 1
                    if pos['Days_Held'] >= 10:
                        # Time Stop executed next open? Or immediately?
                        # Usually time stop is "Exit after N days". Let's exit Next Open to be consistent.
                        pos['Pending_Exit'] = True
        ...
        if len(positions) < MAX_POSITIONS:
            ...
            for symbol, row in candidates.iterrows():
                ...
                # Dynamic Lot Sizing Logic (Target 2%, Max 3%, Lot Size 100)
                LOT_SIZE = 100
                cost_per_lot = entry_price * LOT_SIZE
                
                limit_3pct = curr_eq * 0.03
                target_2pct = curr_eq * 0.02
                
                shares = 0
                
                # Check if we can open at least 1 lot under the 3% cap
                if cost_per_lot <= limit_3pct:
                    # Aim for 2%
                    # Using round() to get nearest integer lot count to target
                    ideal_lots = int(round(target_2pct / cost_per_lot))
                    if ideal_lots < 1: 
                        ideal_lots = 1
                    
                    # Hard cap at 3%
                    # If rounding up pushes us over 3%, or if 2% target logic results in >3% (unlikely given checks, but possible with rounding), clamp it.
                    while (ideal_lots * cost_per_lot) > limit_3pct:
                        ideal_lots -= 1
                    
                    if ideal_lots >= 1:
                        shares = ideal_lots * LOT_SIZE
                        ...
                        if (entry_value + comm_entry) > cash:
                             # Reduce by lots if insufficient cash
                             # Approximate max value available
                             avail_cash = cash - MIN_COMMISSION # rough buffer
                             max_lots_cash = int(avail_cash // (entry_price * (1 + COMMISSION_RATE) * LOT_SIZE))
                             shares = max_lots_cash * LOT_SIZE
...
def main():
    ...
    SCENARIOS = []
    # Dynamic Lot Sizing Scenario
    SCENARIOS.append({
        'label': "Dynamic (Next Open Exit)",
        'size': 0.0, # Placeholder, ignored by new logic
        'sl_config': None
    })
    
    # Add Special SL Case (Optional, keeping as reference or removing if desired, but user focused on sizing)
    # Let's keep one SL variation with dynamic sizing for comparison if user wants, 
    # but to be strict to request "implement this... and amend chart", I will stick to the main request.
    # Actually, I'll keep the SL variant but with the same dynamic sizing implication?
    # The user instruction was specific about sizing. Let's just run the main Dynamic one to be clean.
    ...
    if best_eq_df is not None:
        ...
        # Revert to standard (white) style
        plt.style.use('default')
        
        fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(14, 14), sharex=True, gridspec_kw={'height_ratios': [3, 1, 1]})
        ...
        if not spy_aligned.empty:
            ...
            ax1.plot(spy_aligned.index, spy_aligned['Norm'], label=f"Benchmark", color='gray', alpha=0.5, linestyle='--')
            
        ax1.plot(best_eq_df['Date'], best_eq_df['Equity'], label=f"Strategy", color='#d62728', linewidth=2)
        
        # Formatting Y-Axis for Equity
        ...
        final_date = best_eq_df.iloc[-1]['Date']
        final_equity = best_eq_df.iloc[-1]['Equity']
        total_ret_pct = best_stats['Total Ret %']
        
        ax1.annotate(f"Total Return: +{total_ret_pct:,.0f}%", 
                     xy=(final_date, final_equity), 
                     xytext=(10, 0), textcoords='offset points',
                     ha='left', va='center', fontsize=12, fontweight='bold', color='#d62728')

        title_str = (
            f"Short Parabolic Spike JP Summary (Next Open Exit)\n"
            f"CAGR: {best_stats['CAGR %']:.2f}% | Max DD: {best_stats['Max DD %']:.2f}% | Sharpe: {best_stats['Sharpe']:.2f}\n"
            f"Win Rate: {best_stats['Win Rate %']:.1f}% | Avg Win: {best_stats['Avg Win %']:.1f}% | Avg Loss: {best_stats['Avg Loss %']:.1f}% | PF: {best_stats['Prof Fact']:.2f}"
        )
        ax1.set_title(title_str, fontsize=14, fontweight='bold', pad=20)
        ax1.legend(loc='upper left')
        
        # 2. Exposure
        ax2.fill_between(best_eq_df['Date'], best_eq_df['Num_Positions'], color='purple', alpha=0.3, label="Num Shorts")
        ax2.plot(best_eq_df['Date'], best_eq_df['Num_Positions'], color='purple', linewidth=1)
