...
def get_all_symbols():
    ...
    try:
        data = pd.read_csv("univ_japanese_equities.csv")
        symbols = [str(s)+".T" for s in data.Symbol.to_list()]
        return symbols
    except Exception as e:
        print(f"Could not read symbols from csv: {e}")
        return []
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(script_dir / "b_downdays_LO_comparison_jp.png")
    print(f"\nComparison plot saved to {script_dir / 'b_downdays_LO_comparison_jp.png'}")
