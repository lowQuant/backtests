...
def simulate_decline_days(df, decline_days):
    """
    Run the backtest loop with a specific Decline Days Requirement.
    """
    print(f"\n>>> Simulating with Consecutive Decline Days: {decline_days}")
    ...
    # Logic for Limit Price (Fixed Multiplier = 1.0)
    test_df['Limit_Price_Target'] = test_df['Close'] * (1 + (LIMIT_MULTIPLIER * test_df['Mean_4D_Ret']))
    ...
    for date in tqdm(sim_dates, desc=f"Simulating ({decline_days} days)"):
        try:
...
def main():
    ...
    print("\n" + "="*50)
    print(f"STARTING COMPARISON: CONSECUTIVE DECLINE DAYS")
    print(f"Testing: {DECLINE_DAYS_TO_TEST} days")
    print("="*50)
    ...
    for days in DECLINE_DAYS_TO_TEST:
        stats, eq_df, tr_df = simulate_decline_days(df, days)
        if stats:
            results.append(stats)
            # Save individual runs - COMMENTED OUT
            # eq_df.to_csv(script_dir / f"b_downdays_LO_v2_d{days}_equity.csv", index=False)
            # tr_df.to_csv(script_dir / f"b_downdays_LO_v2_d{days}_trades.csv", index=False)
            
            # Plot Equity Curve
            plt.plot(eq_df['Date'], eq_df['Equity'], label=f"{days} Down Days (Total Ret: {stats['Total Return %']:.0f}%)")
    
    # Finalize Plot
    plt.title("Equity Curve Comparison - Decline Streak Length")
    plt.xlabel("Date")
    plt.ylabel("Equity ($)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(script_dir / "b_downdays_LO_v2_comparison.png")
    print(f"\nComparison plot saved to {script_dir / 'b_downdays_LO_v2_comparison.png'}")
    ...
    if results:
        ...
        print("="*80)
        # Reorder columns
        cols = ["Decline Days", "Final Equity", "Total Return %", "Sharpe", "Max DD %", "Win Rate %", "Avg Trade %", "Trades", "Profit Factor"]
        print(res_df[cols].to_string(index=False))
        print("="*80)
        
        # Save summary
        # res_df.to_csv(script_dir / "b_downdays_LO_v2_comparison.csv", index=False)
