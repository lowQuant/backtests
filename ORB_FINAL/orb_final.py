...
def run_regime_v2_analysis():
    ...
    # ADX Only (No VIX Filter) - For Comparison
    print("\n--- Running ADX Only Strategy (No VIX) ---")
    # To simulate ADX Only using backtest_regime_v2, we can set VIX to always be <= 20 (e.g. 0) in the map
    # This forces "Low Volatility Regime" -> Both Long and Short allowed, effectively just ADX Filter.
    # We create a dummy map where all days have VIX = 0
    vix_map_dummy = {d: 0 for d in days}
    res_adx_only = backtest_regime_v2(days, df, orb_m, target_R, risk, max_Lev, AUM_0, commission, 
                                vix_map_dummy, adx_threshold=20)
    
    # Compare
    res_combined['daily_return'] = res_combined['AUM'].pct_change()
    res_regime['daily_return'] = res_regime['AUM'].pct_change()
    res_adx_only['daily_return'] = res_adx_only['AUM'].pct_change()
    ...
    stats_adx_only = summary_statistics(res_adx_only['daily_return'], res_adx_only['pnl_R'])
    ...
    print(f"\n--- ADX Only (No VIX Filter) Performance ({ticker}) ---")
    print(stats_adx_only.to_string(index=False))
    ...
    plt.plot(res_adx_only['Date'], res_adx_only['AUM'], label='ADX Only (No VIX)', linewidth=1, color='green', linestyle='--')
    plt.yscale('log')
    plt.title(f'5-min ORB Strategy: Regime V2 vs ADX Only\nTicker: {ticker} | ADX>20')
    plt.grid(True, which="both", ls="--")
    plt.legend()
    output_plot = os.path.join(script_dir, 'comparison_regime_v2.png')
