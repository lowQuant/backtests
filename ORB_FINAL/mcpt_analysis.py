...
def extract_daily_features(df):
    ...
    def get_day_stats(g):
        ...
        # R-Multiples
        # Long
        # Entry at Open of 6th bar (matching orb_final.py)
        long_entry = entry_price
        
        # Stop at OR Low
        stop_dist_l = long_entry - or_low
        if stop_dist_l <= 0: stop_dist_l = open_0 * 0.001 
        
        # Check Stop
        long_stopped = post_low <= or_low
        long_r = -1.0 if long_stopped else (moc - long_entry) / stop_dist_l
        
        # Short
        # Entry at Open of 6th bar
        short_entry = entry_price
        
        # Stop at OR High
        stop_dist_s = or_high - short_entry
        if stop_dist_s <= 0: stop_dist_s = open_0 * 0.001
        
        # Check Stop
        short_stopped = post_high >= or_high
        short_r = -1.0 if short_stopped else (short_entry - moc) / stop_dist_s
...
def run_mcpt_v2():
    ...
    if not os.path.exists(vix_path):
        print(f"Error: {vix_path} not found.", flush=True)
        return
    ...
    scenarios = [
        ...
        {'name': 'ORB Strategy (Enhanced)', 'adx': True, 'vix': True, 'type': 'custom'}
    ]
    ...
    for i, sc in enumerate(scenarios):
        ...
        ax.hist(perm_pfs, bins=50, alpha=0.85, color=hist_color, density=True, edgecolor='#0d1b2a', linewidth=0.6)
        ax.axvline(real_pf, color=line_color, linestyle='--', linewidth=2.5, label=f"Real PF: {real_pf:.2f}")
        
        ax.set_title(f"{name} - Monte Carlo Permutation Test\n10,000 Runs | P-Value {p_value:.4f}", 
                     fontsize=15, fontweight='bold', color='white', pad=18)
        ax.set_xlabel("Profit Factor", fontsize=12, fontweight='bold', color='white')
        ax.set_ylabel("Density", fontsize=12, fontweight='bold', color='white')
        
        ax.grid(True, which='major', color='#2f3b52', linestyle='--', linewidth=0.5, alpha=0.6)
        ax.set_facecolor('#010409')
        fig.patch.set_facecolor('#010409')
        
        legend = ax.legend(loc='upper right', frameon=True, facecolor='#0d1117', edgecolor='#2f3b52')
        for text in legend.get_texts():
            text.set_color('white')
        
        for spine in ax.spines.values():
            spine.set_edgecolor('#2f3b52')
            spine.set_linewidth(1)
        
        ax.annotate(f"Perm Mean: {perm_pfs.mean():.2f}\nPerm Std: {perm_pfs.std():.2f}", 
                    xy=(0.02, 0.95), xycoords='axes fraction',
                    fontsize=11, color='white', va='top',
                    bbox=dict(boxstyle='round,pad=0.3', fc='#11151c', ec='#2f3b52', alpha=0.8))
        
        plt.tight_layout()
        
        # Save individual plot (transparent edge to blend on dark backgrounds)
        safe_name = name.replace(" ", "_").replace("(", "").replace(")", "").lower()
        output_plot = os.path.join(script_dir, f'mcpt_result_{safe_name}.png')
    ...
    # Save stats to file
    output_stats = os.path.join(script_dir, 'mcpt_v2_stats.txt')
