...
def trendline_breakout_dataset(
    ...
    atr_indicator = AverageTrueRange(high=np.log(ohlcv['high']), low=np.log(ohlcv['low']), close=np.log(ohlcv['close']), window=atr_lookback)
    atr_arr = atr_indicator.average_true_range().to_numpy()
    ...
    adx_indicator = ADXIndicator(high=ohlcv['high'], low=ohlcv['low'], close=ohlcv['close'], window=lookback)
    adx_arr = adx_indicator.adx().to_numpy()
